#!/bin/bash
usage() {
	echo "$0 <target> <build host> <scp helper>"; exit 1
}
...
ssh -t $buildhost "diff -q config.$target linux/net-next/.config || { make -C linux/net-next distclean; cp config.$target linux/net-next/.config; }"
